...
Window = pygame.display.set_mode((screen_size, screen_size + 100))
    ...
    for i in range (9):
        for j in range (9):
            if defaultgrid [i][j]!= 0:
                ...
                text = font.render(str(defaultgrid [i][j]), 1, (0, 0, 0)) #hiển thị font chữ
                Window.blit(text, (i*diff + 15, j*diff + 15))
    for l in range(10):
        if 1%3 == 0:
            thick = 7
        else:
...
def raiseerror ():
    ...
    Window.blit(text1,(20, screen_size + 20))
...
def validvalue(m, k, l, value):
    ...
    it = k //3
    jt = l //3
    for l in range(it*3, it*3+3):
        for k in range(jt*3, jt*3+3):
            if m[k][l]== value:
                return False
    return True
